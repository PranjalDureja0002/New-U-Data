...
Return ONLY JSON."""
    ...
    def _stage3_context_builder(self, ctx, knowledge, provider, schema_ddl):
        ...
        # NOTE: exclusion_rules and time_filters from knowledge are EXCLUDED intentionally.
        # exclusion_rules cause phantom filters (AMOUNT > 0, SUPPLIER_NO IS NOT NULL, etc.)
        # that the user never asked for.
        # time_filters use SYSDATE/EXTRACT patterns that conflict with the pipeline's
        # temporal context (which provides concrete date literals).
        fr = {"oracle_syntax": rules.get("oracle_syntax", {})}
        ...
        temporal_ctx = (
            ...
            f"    - 'in 2025' / 'for 2025' / 'year 2025' = CALENDAR year → EXTRACT(YEAR FROM INVOICE_DATE) = 2025\n"
            f"    - 'current year' / 'this year' = CALENDAR year → EXTRACT(YEAR FROM INVOICE_DATE) = {_today.year}\n"
            f"    - 'last year' / 'previous year' = CALENDAR year → EXTRACT(YEAR FROM INVOICE_DATE) = {_today.year - 1}\n"
            f"    - 'current fiscal year' / 'FY25' / 'FY 2025' = FISCAL year → INVOICE_DATE >= DATE '{_fy_start}' AND INVOICE_DATE <= DATE '{_fy_end}'\n"
            f"    - 'last fiscal year' / 'FY24' = FISCAL year → INVOICE_DATE >= DATE '{_prev_fy_start}' AND INVOICE_DATE <= DATE '{_prev_fy_end}'\n"
            f"    Use FISCAL year ONLY when user explicitly says 'fiscal year' or 'FY'. Otherwise default to CALENDAR year.\n"
            f"\n  MONTH RULES:\n"
            f"    - 'in June' (month without year, today is {_today.isoformat()}) → INVOICE_DATE >= DATE '2025-06-01' AND INVOICE_DATE <= DATE '2025-06-30'\n"
            f"      Use the MOST RECENT past occurrence. If the month has passed this year, use this year. If not yet, use last year.\n"
            f"    - 'June 2024' (month with year) → INVOICE_DATE >= DATE '2024-06-01' AND INVOICE_DATE <= DATE '2024-06-30'\n"
            f"    - Do NOT use EXTRACT(MONTH FROM INVOICE_DATE) = N without also filtering the year.\n"
            f"\n  OTHER:\n"
            f"    - 'last quarter' → compute previous quarter from current Q{_q}\n"
            f"    - 'last 6 months' → INVOICE_DATE >= ADD_MONTHS(SYSDATE, -6)\n"
            f"    - 'last N months' → INVOICE_DATE >= ADD_MONTHS(SYSDATE, -N)\n"
        )
        ...
        if provider == "oracle":
            sections.append(
                ...
                f"4. Use SYSDATE for relative date calculations (ADD_MONTHS, last N months). For fiscal year boundaries, use the concrete dates from Temporal Context.\n"
        ...
        sections.append(f"\n**Rules:**\n1. SELECT only\n2. Use exact column names\n3. GROUP BY for aggregations\n4. Return ONLY the SQL, no explanations\n5. For name/text filters (SUPPLIER_NAME, PLANT_NAME, etc.), use UPPER(col) LIKE UPPER('%value%') for partial matching, NOT exact equality\n6. Do NOT add filters the user did not ask for:\n   - NO phantom filters like AMOUNT > 0, AMOUNT IS NOT NULL, SUPPLIER_NO IS NOT NULL, PLANT_NO IS NOT NULL\n   - NO extra dimension filters (REGION, COUNTRY, etc.) unless user mentions them\n   - The ONLY mandatory filter is INVOICE_DATE > DATE '2024-04-01' (injected automatically)\n7. 'in 2025' = calendar year (Jan-Dec). Only use fiscal year when user explicitly says 'fiscal year' or 'FY'.")
    ...
    def _build_interactive_table(self, columns, rows):
        ...
        for row_idx, row in enumerate(rows):
            ...
            for i, val in enumerate(row):
                ...
                lines.append(f'<td style="padding:7px 12px;border-bottom:1px solid #f1f5f9;text-align:{align};{font}">{_fmt(val)}</td>')
            lines.append('</tr>')
